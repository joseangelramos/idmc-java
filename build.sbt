...
lazy val server = (project in file("server"))
  ...
  .settings (
    ...
    Compile / sourceGenerators += Def.task {
      val file = (Compile / sourceManaged).value / "io" / "idmc" / "server" / "utils" / "VersionUtils.java"
      IO.write(file,
        s"""package io.idmc.server.utils;
           |
...
lazy val cli = (project in file("examples") / "cli")
  ...
  .settings(
    ...
    mainClass := Some(orgName + ".cli.IDMCCli"),
    commonSettings,
    javaCheckstyleSettings(file("dev") / "checkstyle-config.xml"),
